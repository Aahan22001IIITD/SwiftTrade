...
/******************************************************************************
 * This is OPTMISED DOCUMENTED Version of the dom.cpp
 *****************************************************************************/
...
{
    {
        string filename = "Optimised application_logs.txt";
    ...
    {
        string filename = "Optimised Latency Log.txt";
        ofstream file(filename, ios::trunc);
        if (file.is_open())
        {
...
{
    static ofstream logFile("Optimised application_logs.txt", ios::app);
    if (logFile.is_open())
    {
...
{
    static ofstream logFile("Optimised Latency Log.txt", ios::app);
    if (logFile.is_open())
    {
        logFile << message << endl;
    }
...
static CurlGlobalInit curlInitGuard; // Initialize once for all instances
...
public:
    DeribitApi(const string &client_id, const string &client_secret)
        : clientId_(client_id), clientSecret_(client_secret), curl_(nullptr), persistentHeaders_(nullptr)
    {
        curl_global_init(CURL_GLOBAL_ALL);
        curl_ = curl_easy_init();
        if (curl_)
        {
            persistentHeaders_ = curl_slist_append(persistentHeaders_, "Content-Type: application/json");
            ...
            curl_easy_setopt(curl_, CURLOPT_SSL_SESSIONID_CACHE, 1L);
    ...
    ~DeribitApi()
    {
        ...
        if (persistentHeaders_)
        {
            curl_slist_free_all(persistentHeaders_);
        }
        curl_global_cleanup();
    }
    /**
    ...
    bool refreshToken()
    {
        ...
        json requestBody = {
            ...
            {"id", 1},
            {"method", "public/auth"},
            {"params", {{"grant_type", "refresh_token"}, {"refresh_token", refreshToken_}}}};

        string response;
        setCurlOptions(url, requestBody.dump(), response);

        try
        {
            static json jsonResp;
            jsonResp = json::parse(response);
            if (jsonResp.contains("error"))
            {
                cerr << "[Auth] Error in response: " << jsonResp["error"]["message"] << endl;
                return false;
            }
            ...
            tokenExpiresIn_ = result["expires_in"].get<int>();
            tokenIssueTime_ = chrono::system_clock::now();
            cout << "[Auth] Successfully refreshed token." << endl;
            return true;
        }
        catch (const exception &e)
        {
            cerr << "[Auth] Exception while parsing response: " << e.what() << endl;
            return false;
        }
    }
     ...
     */
    string placeOrder(const string &side, const string &instrument_name, const string &type, double amount, double price = 0.0)
    {
        ...
        string url = DERIBIT_REST_HOST + endpoint + "?instrument_name=" + instrument_name + "&amount=" + to_string(amount);
        ...
        logMessage("[placeOrder]: " + url);

        string response;
        setCurlOptions(url, "", response);
        return response;
    }
...
private:
    void setCurlOptions(const string &url, const string &postData, string &response)
    {
        if (!curl_)
        {
            cerr << "[Error] CURL handle is not initialized." << endl;
            return;
        }
        ...
        curl_easy_setopt(curl_, CURLOPT_HTTPHEADER, persistentHeaders_);
    }

    string executePostRequest(const string &url)
    {
        string response;
        CurlHandle curl; // RAII wrapper for CURL handle
        if (!curl.get())
        {
    ...
    CURL *curl_;
    struct curl_slist *persistentHeaders_;
    mutex tokenMutex;
};
...
public:
    ...
    void connect()
    {
        ...
        try
        {
            ...
            ws_.binary(false); // Enable text frames for better compatibility
...
public:
    ...
    public:
        ...
        void start()
        {
            ...
            ws_.set_option(WebSocket::stream_base::timeout::suggested(Beast::role_type::server)); // Optimized timeouts
    ...
    void cleanupResources()
    {
        ...
        logMessage("[Local WS] All sessions and subscriptions have been cleaned up.\n");
    }
...
private:
    ...
    unordered_map<shared_ptr<Session>, set<string>> clientSubscriptions_;
    Net::io_context ioc_;
    unordered_map<string, string> lastData_;
    tcp::acceptor acceptor_;
    thread serverThread_;
    mutex mutex_;
    unordered_map<string, vector<weak_ptr<Session>>> subscriptions_;
};

// Global context for cleanup (using pointers to allow cleanup)
static SimpleWebSocketServer *globalServer = nullptr;
static unordered_map<string, unique_ptr<DeribitMarketData>> *globalActiveStreams = nullptr;
...
int main()
{
    ...
    client.connect();
    unordered_map<string, string> orderMap; // Tracks bought instruments with order IDs
    if (!api.authenticate())
    {
    ...
    while (true)
    {
        ...
        case 1:
        {
            ...
            vector<tuple<string, string, double, double>> orders = {
                ...
                {"BTC-PERPETUAL", "limit", 10, 30000.0} // Limit order example
            };
        ...
        case 5: // Get Positions
        {
            ...
            logMessage("[Info]: Get Positons Response for instrument name: " + instrumentName + " is " + response + "\n");
            if (!response.empty())
            {
                cout << "Positions:\n"
                     << response << "\n";
        ...
        case 6: // Start WebSocket Server
        {
            ...
            try
            {
                ...
                unordered_map<string, unique_ptr<DeribitMarketData>> activeStreams;
                globalServer = &server;
                ...
                while (running)
                {
                    ...
                    for (const auto &instrument : server.getSubscribedInstruments())
                    {
                        if (activeStreams.find(instrument) == activeStreams.end())
                        {
                            ...
                            activeStreams[instrument] = std::make_unique<DeribitMarketData>(
                                DERIBIT_WS_HOST,
                                DERIBIT_WS_PORT,
                                instrument,
                                accessToken,
                                [&server, instrument](const std::string_view &msg) { // Updated to string_view
                                    try
                                    {
                                        server.onOrderBookUpdate(instrument, std::string(msg));
                                        logMessage("[Info] Order book update broadcasted for instrument: " + instrument);
                                    }
                                    catch (const std::exception &e)
                                    {
                                        std::cerr << "[Error] Failed to broadcast order book update for " << instrument
                                                  << ": " << e.what() << "\n";
                                        logMessage("[Error] Exception during order book update: " + std::string(e.what()));
                                    }
                                });
